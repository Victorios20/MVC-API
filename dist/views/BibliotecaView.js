"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BibliotecaView = void 0;
class BibliotecaView {
    constructor(controller) {
        this.controller = controller;
    }
    mostrarLivros() {
        return __awaiter(this, void 0, void 0, function* () {
            const livros = yield this.controller.listar();
            console.log("Livros disponíveis na biblioteca:");
            livros.forEach(livro => console.log(`${livro.titulo} (${livro.id}) - ${livro.status}`));
        });
    }
    buscarLivro(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const livro = yield this.controller.buscar(id);
            if (livro) {
                console.log(`Livro encontrado: ${livro.titulo} por ${livro.autor}`);
            }
            else {
                console.log("Livro não encontrado.");
            }
        });
    }
    reservarLivro(livro) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.controller.adicionar(livro);
            console.log(`Livro ${livro.titulo} reservado com sucesso.`);
        });
    }
    cancelarReserva(id) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.controller.remover(id);
            console.log("Reserva cancelada com sucesso.");
        });
    }
}
exports.BibliotecaView = BibliotecaView;
