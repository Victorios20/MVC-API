...
class Disciplina {
    ...
    listarDisciplinas() {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield axios_1.default.get(BASE_URL_DISCIPLINA);
            if (!Array.isArray(response.data)) {
                console.error("Os dados retornados não são um array:", response.data);
                return [];
            }
            return response.data.map((disciplina) => new Disciplina({ id: disciplina.id, curso: disciplina.curso, nome: disciplina.nome }));
        });
    }
}
